...
if __name__ == "__main__":
    ...
    for i, (date, name) in enumerate(multipages_career(N)):
        print('({0}) {1}:  {2}'.format(i+1, name, date))     # print the entire results, job name + post date
    
    dayjobdict = dayjobcount(multipages_career(N))       # count the number of total job for each day
